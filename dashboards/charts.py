...
def risk_policy_chart(
    rp: pd.DataFrame,
    title: str | None = None,
) -> go.Figure:
    """Horizontal bars of model-minus-naive uplift per dispatch policy on
    the 61-day holdout. `rp` is the wide xgboost_battery_pnl_policies.csv.

    Story: greedy P50 ties or beats every band-aware variant; skip-wide-
    slots is catastrophic. Worst-day annotation gives the second axis
    without a second chart.
    """
    POLICY_ORDER = [
        ("greedy_p50",      "Greedy P50",                  PREDICTION),
        ("p10p90",          "P10 charge / P90 discharge",  PREDICTION),
        ("width_penalty",   "Width penalty (λ=0.1)",       PREDICTION),
        ("robust_p90p10",   "Robust P90 / P10",            TSO),
        ("skip_wide_slots", "Skip top-20 % widest slots",  "#d97c7c"),
    ]
    naive_total = float(rp["naive_pnl"].sum())
    labels, uplifts, colors, worst_text = [], [], [], []
    for key, label, color in POLICY_ORDER:
        col = f"pnl_{key}"
        ...
        colors.append(color)
        worst_text.append(
            f"worst day: €{worst:+,.0f} · "
            f"days naive won: {lost}/{len(rp)}"
        )

    fig = go.Figure()
    fig.add_vline(x=0, line=dict(color=TEXT_30, width=1, dash="dot"))
    fig.add_trace(go.Bar(
        x=uplifts,
        y=labels,
        orientation="h",
        marker=dict(color=colors),
        text=[f"€{u:+,.0f}" for u in uplifts],
        textposition="auto",
        hovertext=worst_text,
        hovertemplate="%{x:,.0f} € uplift vs naive<br>%{hovertext}<extra>%{y}</extra>",
        showlegend=False,
    ))
    layout = _base_layout(title=title, height=320)
    layout["xaxis"] = {**_AXIS, "title": "Total uplift vs naive (€ over 61 days)"}
    layout["yaxis"] = {**_AXIS, "autorange": "reversed"}  # best policy on top
    fig.update_layout(**layout)
    return fig
...
def conformal_chart(
    ...
    """Side-by-side: (left) coverage moves under calibration, (right)
    dispatch uplift is invariant. Conformal works on coverage; greedy P50
    wins all three band variants by an identical margin.
    """
    from plotly.subplots import make_subplots
    fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=("80%-band coverage", "Greedy P50 dispatch uplift"),
        column_widths=[0.45, 0.55],
        horizontal_spacing=0.12,
    )

    # Left: coverage bars
    cov_pre = float(summary["test_coverage_uncalibrated"]) * 100
    cov_marg = float(summary["test_coverage_marginal"]) * 100
    cov_adapt = float(summary["test_coverage_adaptive"]) * 100
    fig.add_trace(go.Bar(
        x=["uncalibrated", "marginal", "adaptive"],
        y=[cov_pre, cov_marg, cov_adapt],
        marker=dict(color=[TSO, PREDICTION, PREDICTION]),
        text=[f"{cov_pre:.1f}%", f"{cov_marg:.1f}%", f"{cov_adapt:.1f}%"],
        textposition="outside",
        showlegend=False,
        hovertemplate="%{x}<br>%{y:.1f}%<extra></extra>",
    ), row=1, col=1)
    fig.add_hline(y=80, line=dict(color=TEXT_50, width=1, dash="dot"),
                  annotation_text="target 80 %",
                  annotation_position="top right",
                  annotation_font=dict(color=TEXT_50, size=10),
                  row=1, col=1)

    # Right: dispatch uplift across (band variant × policy)
    BANDS = ["uncalibrated", "marginal", "adaptive"]
    POLICIES = [
        ("greedy_p50", "Greedy P50", PREDICTION),
        ("p10p90", "P10/P90", "#8c6dff"),
        ("width_penalty", "Width pen.", "#6537d6"),
    ]
    for key, label, color in POLICIES:
        sub = cf_pol[cf_pol["policy"] == key].set_index("bands")
        values = [float(sub.loc[b, "uplift_vs_naive"]) if b in sub.index else 0
                  for b in BANDS]
        fig.add_trace(go.Bar(
            x=BANDS, y=values,
            ...
            hovertemplate=f"<b>{label}</b><br>%{{x}}<br>%{{y:+,.0f}} € uplift<extra></extra>",
        ), row=1, col=2)

    layout = _base_layout(title=title, height=380)
    layout["yaxis"] = {**_AXIS, "title": "Coverage (%)",
                       "range": [60, 100]}
    layout["yaxis2"] = {**_AXIS, "title": "Uplift vs naive (€, test split)"}
    layout["xaxis"] = {**_AXIS}
    layout["xaxis2"] = {**_AXIS}
    layout["barmode"] = "group"
    layout["legend"] = {**layout["legend"], "y": 1.08}
    # subplot title styling
    fig.update_annotations(font=dict(size=11, color=TEXT_70))
    fig.update_layout(**layout)
    return fig


def da_vs_intraday_chart(
    daily: pd.DataFrame,
    title: str | None = None,
) -> go.Figure:
    """Cumulative dispatch P&L on each market — same architecture,
    two targets. Three lines per market: oracle (ceiling), model,
    naive. The visual story: model > naive on both markets; intraday
    has lower oracle but the model captures comparable share.
    """
    from plotly.subplots import make_subplots
    fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=("Day-ahead market", "Intraday continuous market"),
    ...
    daily = daily.sort_values("issue_date")
    x = daily["issue_date"]

    # Day-ahead
    fig.add_trace(go.Scatter(
        x=x, y=daily["da_oracle_pnl"].cumsum(),
        mode="lines", line=dict(color=TEXT_50, width=1.5, dash="dot"),
        name="Perfect-foresight (oracle)",
        hovertemplate="%{x|%b %d}<br>€%{y:,.0f}<extra>DA oracle</extra>",
    ), row=1, col=1)
    fig.add_trace(go.Scatter(
        x=x, y=daily["da_model_pnl"].cumsum(),
        mode="lines", line=dict(color=PREDICTION, width=2.4),
        name="Model P50",
        hovertemplate="%{x|%b %d}<br>€%{y:,.0f}<extra>DA model</extra>",
    ), row=1, col=1)
    fig.add_trace(go.Scatter(
        x=x, y=daily["da_naive_pnl"].cumsum(),
        mode="lines", line=dict(color=ACTUAL, width=1.6, dash="dash"),
        name="Naive yesterday",
        hovertemplate="%{x|%b %d}<br>€%{y:,.0f}<extra>DA naive</extra>",
    ), row=1, col=1)

    # Intraday
    fig.add_trace(go.Scatter(
        x=x, y=daily["id_oracle_pnl"].cumsum(),
        mode="lines", line=dict(color=TEXT_50, width=1.5, dash="dot"),
        name="Perfect-foresight (oracle)",
        showlegend=False,
        hovertemplate="%{x|%b %d}<br>€%{y:,.0f}<extra>ID oracle</extra>",
    ), row=1, col=2)
    fig.add_trace(go.Scatter(
        x=x, y=daily["id_model_pnl"].cumsum(),
        mode="lines", line=dict(color=PREDICTION, width=2.4),
        name="Model P50",
        showlegend=False,
        hovertemplate="%{x|%b %d}<br>€%{y:,.0f}<extra>ID model</extra>",
    ), row=1, col=2)
    fig.add_trace(go.Scatter(
        x=x, y=daily["id_naive_pnl"].cumsum(),
        mode="lines", line=dict(color=ACTUAL, width=1.6, dash="dash"),
        name="Naive yesterday",
        showlegend=False,
        hovertemplate="%{x|%b %d}<br>€%{y:,.0f}<extra>ID naive</extra>",
    ), row=1, col=2)

    layout = _base_layout(title=title, height=380)
    layout["yaxis"] = {**_AXIS, "title": "Cumulative dispatch P&L (€)"}
    layout["yaxis2"] = {**_AXIS, "title": "Cumulative dispatch P&L (€)"}
    layout["xaxis"] = {**_AXIS, "title": "Delivery day"}
    layout["xaxis2"] = {**_AXIS, "title": "Delivery day"}
    layout["hovermode"] = "x unified"
    fig.update_annotations(font=dict(size=11, color=TEXT_70))
    fig.update_layout(**layout)
    return fig
