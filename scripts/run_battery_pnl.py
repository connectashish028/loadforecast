...
Reads the **production XGBoost** price holdout; output is what the dashboard's
battery-P&L panel reads. The historical LSTM-era artifacts
(`price_quantile_holdout.csv`, `battery_pnl_daily.csv`) are preserved as the
comparison baseline but are no longer regenerated by this script.

Output: backtest_results/xgboost_battery_pnl_daily.csv  (one row per day)
"""
...
OUT_CSV = Path("backtest_results/xgboost_battery_pnl_daily.csv")
