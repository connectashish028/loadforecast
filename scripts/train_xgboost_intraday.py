"""Train an XGBoost quantile model targeting the German INTRADAY
continuous average price (SMARD filter 252).

Phase B.4 step 2. Mirrors `train_xgboost_price.py` exactly — same
architecture, same 50 features (47 base + 3 engineered VRE), same
sample weights, same train/val/holdout splits. The ONLY change is
the target column:

    DA:       price__germany_luxembourg          (cleared once at D-1 12:45)
    intraday: price__intraday_continuous_de_lu   (volume-weighted avg of
                                                  continuous trades per
                                                  delivery slot)

Honest question this answers: with the same features and same model
class that hit 17.7 EUR/MWh P50 MAE on day-ahead, **what skill does
the same setup achieve on the materially harder intraday-continuous
target?** Higher MAE is expected (intraday std basis vs DA is 26 EUR/
MWh — there's that much extra noise to forecast). The interesting
metric is not absolute MAE but skill vs naive intraday-yesterday
baseline.
"""
...
OUT_DIR = Path("model_checkpoints/xgboost_intraday_v1")
TARGET_COL = "price__intraday_continuous_de_lu"
...
def build_dataset(df: pd.DataFrame, dates: list[date]) -> tuple[np.ndarray, np.ndarray, np.ndarray, list[str]]:
    ...
    for d in dates:
        ...
        target = df[TARGET_COL].reindex(features.index).to_numpy()
    ...
    if not X_list:
        raise RuntimeError("No usable training samples — check date range and target column.")
    return (
        np.vstack(X_list),
...
def main() -> None:
    ...
    if TARGET_COL not in df.columns:
        raise SystemExit(
            f"Target column {TARGET_COL!r} not in parquet. Run "
    ...
    print(f"\nBuilding datasets (target = {TARGET_COL}) ...")
    ...
    n_upweighted = int((w_train > 1.5).sum() // 96)
    ...
    meta = {
        "model": "xgboost_intraday_v1",
        "target": f"raw intraday continuous average price (EUR/MWh) [{TARGET_COL}]",
